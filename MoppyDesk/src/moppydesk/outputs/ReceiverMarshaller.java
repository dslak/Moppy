...
/**
 * Marshals data from the chosen input device to the (up to) 12 output receivers.
 * Receivers are defined in a size-12 array, one slot for each MIDI channel.  Each received
...
public class ReceiverMarshaller implements MoppyReceiver{
    ...
    public void setReceiver(int MIDIChannel, MoppyReceiver channelReceiver){
        if (MIDIChannel < 1 || MIDIChannel > 12){
            throw new IllegalArgumentException("Only channels 1-12 are supported by the ReceiverMarshaller!");
        }
